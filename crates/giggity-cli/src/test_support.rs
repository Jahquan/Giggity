...
mod tests {
    ...
    fn env_var_guard_sets_and_restores_values() {
        let original = std::env::var_os("GIGGITY_TEST_ENV");
        {
            let _guard = EnvVarGuard::set("GIGGITY_TEST_ENV", "set");
            assert_eq!(std::env::var("GIGGITY_TEST_ENV").as_deref(), Ok("set"));
        }
        assert_eq!(std::env::var_os("GIGGITY_TEST_ENV"), original);
    }
